...
export function prepareBackupDirectory(): DirectoryPath {
  ...
  const backupDirectory = (config.backup.directory +
    directoryName) as DirectoryPath;
...
export function backupDirectoryCleanup(): void {
  ...
  let backupsToCleanup = execSync(
    `ls -1 -tr ${config.backup.directory} | head -n -${config.backup.keepAmount}`
  )
    .toString()
    .trim()
    .split("\n")
...
export function backupDirectoryDeleteAll(): void {
  ...
  // We need to use system ls, because node's readdir misses sort by date ability
  let backupsToCleanup = execSync(`ls -1 -tr ${config.backup.directory}`)
    .toString()
    .trim()
    .split("\n")
    .filter((n) => n);
...
export function checkUtilitesAvailability(checkOnSite: boolean = false) {
  ...
  try {
    ...
    if (checkOnSite) {
      throw Error(
        `Site "${siteUpstreamId}" is missing one of those utilites: ${utilitesList}`
      );
    } else {
