...
function configInit() {
  try {
    ...
    if (config.dumpFile.indexOf("{{TIMESTAMP}}")) {
      config.dumpFile = config.dumpFile.replace('{{TIMESTAMP}}', Date.now());
    }
    config.siteCurrent = {};
...
function sshCredentials(site = null) {
  site = site ?? config.siteUpstreamName;
  return {
    host: config.sites[site].host,
    username: config.sites[site].user,
