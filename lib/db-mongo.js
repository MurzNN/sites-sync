...
module.exports = {
  ...
  dbClear: function (dbConnectionString) {
    restoreCommand = `mongo --uri=${dbConnectionString}  --eval 'db.getCollectionNames().forEach(function(c) { if (c.indexOf("system.") == -1) db[c].drop(); })'`;
    execSync(restoreCommand);
  },
};
