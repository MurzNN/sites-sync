...
module.exports = {
  dumpOutput: function (db) {
    const execSync = require("child_process").execSync;
    dumpCommand = `PGPASSWORD=${db.password} pg_dump ` + pgOptions(db) + ` | grep -v -E '^(CREATE\ EXTENSION|COMMENT\ ON)' | gzip -f`;
  ...
  dumpToFile: function (db, dumpFile) {
    ...
    dumpCommand = `PGPASSWORD=${db.password} pg_dump ` + pgOptions(db) + ` | grep -v -E '^(CREATE\ EXTENSION|COMMENT\ ON)' | gzip -f > ${dumpFile}`;
